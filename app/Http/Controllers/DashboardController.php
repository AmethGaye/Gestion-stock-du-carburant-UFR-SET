...
class DashboardController extends Controller
{
    ...
    public function index(){
        ...
        if(auth()->user()->role == 'chef_departement' || auth()->user()->role == 'assistant'){
            ...
            //Pour les cours
            $sceance_cours_non_approuve=Cours::whereHas('matiere.filiere.departement',function ($query) use ($id_user){
                                  $query->where('user_id',$id_user);
                              })->with('matiere','vacataire')->where('statut',0)->count();
            $total_demandes = Cours::whereHas('matiere.filiere.departement',function ($query) use ($id_user){
                $query->where('user_id',$id_user);
            })->with('matiere','vacataire')->where('demande',1)->count();

            $total_cours_added = Cours::whereHas('matiere.filiere.departement',function ($query) use ($id_user){
                $query->where('user_id',$id_user);
